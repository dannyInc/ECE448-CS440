...
def smooth_twTable(tag_word_table, smoothing_param, appear_once):
...
def viterbi_2(train, test):
    ...
    # count probability for hapax words
    # tag:count
    appear_once = dict() 
    for tag in tag_word_table.keys():
        count = 0
        for word in tag_word_table[tag].keys():
            if tag_word_table[tag][word] == 1:
                count += 1
        appear_once[tag] = count

    # Laplace-smoothing/computing log probabilities for the tag-word table
    smooth_twTable(tag_word_table, smoothing_param_emis, appear_once)
